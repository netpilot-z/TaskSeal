...
当前不能直接进入飞书资源准备或 Adapter 实现。`NP-13` 的两个约束存在冲突：
...
推荐修改 `NP-13`：允许为专用测试应用启用机器人能力，但不申请消息 API 权限、
不订阅消息事件、不发送消息，只把应用作为 Base 的文档访问主体。这个选择保留
应用身份、短期 tenant token 和无人值守执行模型，也不授予记录创建、更新或删除
能力。
...
| 方案 | 优势 | 代价 | 主要风险 | 适用条件 |
| --- | --- | --- | --- | --- |
| A. tenant token + 仅启用机器人能力作为文档应用（推荐） | 无人值守；短期 token；权限可收敛到 Base 只读 scope | 需要修订 `NP-13` 的“无机器人”字面约束 | 若误加消息权限或事件订阅会扩大能力 | 接受“启用能力但零消息权限、零消息行为” |
...
只有在操作者批准方案 A 后，专用测试应用才申请以下细粒度权限：
...
1. 由操作者确认方案 A 或 B；建议 A。
2. 若选择 A，更新 `NP-13`，把“无机器人”改为“零消息权限、零消息订阅、零消息行为”。
3. 按最小 scope 和环境契约准备专用资源；只在 Linear 评论非敏感坐标或变量已配置事实。
4. 资源就绪后再把 `NP-11` 移入 Todo/In Progress，并实现 token、field、page、
