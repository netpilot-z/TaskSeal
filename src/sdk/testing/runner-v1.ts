...
export function registerRunnerAdapterContract({
  ...
  test(
    `${name} exposes a valid v1 capability manifest`,
    () => {
      const adapter =
        createAdapter("completed");
      assert.deepEqual(
        parseRunnerManifest(
          adapter.manifest
        ),
  ...
  test(
    `${name} returns a decodable output bound to the host attempt`,
    async () => {
      const adapter =
        createAdapter("completed");
      const manifest =
  ...
  test(
    `${name} settles after the host aborts execution`,
    async () => {
      const adapter =
        createAdapter("cancel");
      const controller =
        new AbortController();
      const execution =
        Promise.resolve(
