...
27. Linear Issue 的可执行归属由当前仓库配置的 `linear.project` 决定；workspace、team、identifier 前缀或人工传入 UUID 都不能扩大范围。ready-work、显式 inspection 与 apply-time provenance 必须分别在读取、snapshot 和提交前校验同一 Project，漂移时失败关闭。
