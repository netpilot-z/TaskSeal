...
自 2026-07-28 起，Linear 是内部产品研发任务的权威来源；GitHub Issue 只承载外部 Bug、公开反馈和仓库级问题，GitHub PR/Review/CI 继续承载代码交付。仓库 `docs/tickets/` 是可审查的规格拆分与 bootstrap 输入，不是第二套在线状态；`NP-2`～`NP-12` 已按预存 UUID 创建并回读核验，映射见 `0007-linear-bootstrap-map.json`，默认 `0006` manifest 当前为空。GitHub 规划 Issue `#7`、`#25`、`#32` 已留下 Linear 链接并迁移关闭。Issue 创建、更新和评论权限已验证；`issueRelationCreate` 仍因缺少通用 `write` scope 而失败关闭，因此原生依赖关系尚未建立。Operation v2 已在 fake transport 边界绑定 Project/State/Parent/source intent；产品内自动 mutation 仍未接入 CLI、HTTP、浏览器或真实 HTTP exchange composition，不能旁路 v2 审批合同。
...
Provider Operation Journal v1 envelope 已使用固定 `.taskseal/provider-operations.json` 保存受控写的完整 version 历史。它通过 reader-first union 逐条解析 Operation v1/v2、逐对验证同版本相邻状态转换，以 `expectedVersion + operationKey + planDigest` 在单实例队列内 compare-and-append；exact latest retry 只返回 idempotent。v1 persisted golden bytes 与摘要保持不变；同一 client UUID 跨版本复用形成 plan conflict。文件采用 16 MiB / 512 records 硬边界与原子 whole-file replace，rename 后按已 sync temp identity 复核 target，未知结果会 fence 当前实例；合法 orphan temp 可经 identity/single-link 检查复用。当前仅通过 query port 接入只读 Control Room projection，没有 CLI/HTTP command 或真实 Linear mutation；首条 v2 record 落盘后只能回滚到具备 union reader 的版本。多进程 CAS、密码学防删改和不可信本地并发写者不在首版保证内，pathname rename 也不承诺对同权限恶意跨进程替换零越界副作用。

Fake Linear Write Transport 保留 v1 `createIssue/queryByClientUuid` exact port，并新增独立 v2 `createIssueV2/queryByClientUuidV2` port。v2 create 使用 client UUID 作为 Issue ID，显式发送 Team/Project/State/Parent，并从响应观察 Team Organization、Team、Project、State 与 Parent；任何 identity/placement 漂移分别进入 outcome unknown 或 reconciliation ambiguous。两个版本都只能显式注入 exchange，没有 global fetch 或凭证 fallback。现有固定 endpoint、单凭证、15 秒 timeout、128 KiB request、64 KiB streaming response 的 GraphQL HTTP exchange仍未组合进真实 mutation；只读 introspection 已确认当前 Linear schema 存在所需字段。

Controlled Write Coordinator 已保留 v1 行为并新增 `prepareV2` 与可选 v2 transport。Project/State/Parent/source intent 在 prepare 时进入不可变 plan；submit/reconcile 只从 committed v2 snapshot 显式投影 transport 参数，source intent 不出站。缺少 v2 port 时在 begin append 前失败，保持零 journal advance 和零 permit。per-operation queue、response-lost fence、client UUID reconciliation 与 reopen 零 transport recovery 对 v1/v2 一致。当前没有 CLI/HTTP、浏览器审批、global fetch、真实凭证 composition 或真实 Linear mutation。
...
| `OperationPlan` | 一次受控外部写的不可变意图。v2 绑定固定 capability/action、持久 client UUID、configured Project/State、resolved Organization/Team/Project/State/Parent、source intent、payload 与 plan digest。 |
