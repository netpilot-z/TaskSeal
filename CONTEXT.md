...
自 2026-07-28 起，Linear 是内部产品研发任务的权威来源；GitHub Issue 只承载外部 Bug、公开反馈和仓库级问题，GitHub PR/Review/CI 继续承载代码交付。仓库 `docs/tickets/` 是可审查的规格拆分与 bootstrap 输入，不是第二套在线状态；默认 dry-run 只读取未完成且尚未映射的 `0006-linear-bootstrap-manifest.md`，并过滤任意 source 中的已完成状态。操作者已为当前 Linear 凭证补充 Issue 创建、更新和评论权限，并通过 `NP-1` 的精确 UUID 更新与读后核验确认生效；TaskSeal 产品内的自动 mutation 仍必须等待 Operation v2 绑定 Project/State/source intent，不能旁路注入 v1。
